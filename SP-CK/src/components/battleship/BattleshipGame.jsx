...
export const BattleshipGame = ({ onBack }) => {
    ...
    useEffect(() => {
        ...
        if (shotResult.hitter !== gameState.opponent) {
            ...
            if (shotResult.result === 'sunk') newMessage = `Chính xác! Đã bắn chìm thuyền ${shotResult.shipType} của đối thủ!`;
        } else {
            if (shotResult.result === 'hit') newMessage = `Họ đã bắn trúng!`;
            if (shotResult.result === 'sunk') newMessage = `Họ đã bắn chìm thuyền ${shotResult.shipType} của bạn!`;
        }
                ...
                return (
                    <div className="flex flex-col items-center">
                        <div className="h-10 mb-4 text-center">
                            {message && <p className="text-xl text-yellow-300 animate-pulse">{message}</p>}
                        </div>
                        {gameState.isMyTurn ? (
                            <CombatGrid
                                board={gameState.opponentBoard}
                                title={`Tấn công ${gameState.opponent}`}
                                onCellClick={handleFireShot}
                            />
                        ) : (
                            <CombatGrid
                                board={gameState.myBoard}
                                title="Hạm đội của bạn (Chờ đối thủ)"
                                onCellClick={() => {}}
                                isMyBoard={true}
                            />
                        )}
